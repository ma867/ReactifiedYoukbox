...
import Songs from "./src/pages/SongsPage/Songs";
import Playlists from "./src/pages/PlaylistsPage/Playlists";
import Playlist from "./src/pages/PlaylistPage/Playlist";
...
function App() {
  ...
  const [updatedPlaylistArtwork, setUpdatedPlaylistArtwork] = useState(
    playlist?.artwork
  );
  ...
  const addPlaylistSong = async (songData, id) => {
    try {
      const response = await fetch(
        `/api/songs/playlistSong/${id}/u/${user?._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteSpotifyPlaylistSong = async (id, songId) => {
    try {
      await fetch(
        `/api/songs/playlistId/${id}/songId/${songId}/user/${user?._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const updatePlaylist = async (id, updatedData) => {
    try {
      const response = await fetch(`/api/playlists/${id}`, {
        ...
        body: JSON.stringify({
          ...updatedData,
          artwork: updatedPlaylistArtwork,
        }),
      });
      const data = await response.json();
    } catch (error) {
